// This code is derived from the FastCRC repository, which can be found here:
// https://github.com/yubabich/FastCRC
//
// The license for it may be found here:
// https://github.com/XJDHDR/random-code/blob/main/C_Sharp_Projects/CRC_32C_Calculator/Models/SoftwareCrc32C/LICENSE
//
...
namespace CRC_32C_Calculator.Models.SoftwareCrc
{
	/// <summary>
	...
	/// Perf tests : this implementation is just little bit slower (about 10%) than native CLI C++ fast Crc32 implementation:
	/// 96 buffers by 128MB (12GB) data - 5.293 seconds vs 4.910 seconds
	/// </summary>
	public static class SliceBy16
	{
		// ==== Public methods ====
		...
		public static uint Crc32(ReadOnlyMemory<byte> Data) =>
			Crc32(Data, Data.Length, 0);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(ReadOnlyMemory<byte> Data, int Length) =>
			Crc32(Data, Length, 0);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(ReadOnlyMemory<byte> Data, uint PreviousCrc32) =>
			Crc32(Data, Data.Length, PreviousCrc32);

		public static uint Crc32(ReadOnlyMemory<byte> Data, int Length, uint PreviousCrc32)
		{
			using (MemoryHandle memHandle = Data.Pin())
			{
				unsafe
				{
					...
					return crc32(dataPtr, Length, PreviousCrc32);
				}
			}
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(ReadOnlySpan<byte> Data, int Length) =>
			Crc32(Data, Length, 0);


		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(ReadOnlySpan<byte> Data, uint PreviousCrc32) =>
			Crc32(Data, Data.Length, PreviousCrc32);

		public static uint Crc32(ReadOnlySpan<byte> Data, int Length, uint PreviousCrc32)
		{
			unsafe
			{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(int Offset, byte[] Data) =>
			Crc32(Offset, Data, Data.Length, 0);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(byte[] Data, int Count) =>
			Crc32(0, Data, Count, 0);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(byte[] Data, uint PreviousCrc32) =>
			Crc32(0, Data, Data.Length, PreviousCrc32);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(int Offset, byte[] Data, uint PreviousCrc32) =>
			Crc32(Offset, Data, Data.Length, PreviousCrc32);

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static uint Crc32(byte[] Data, int Count, uint PreviousCrc32) =>
			Crc32(0, Data, Count, PreviousCrc32);

		public static uint Crc32(int Offset, byte[] Data, int Count, uint PreviousCrc32)
		{
			if (Data == null)
			{
		...
		// ==== Private methods ====
		private static unsafe uint crc32(byte* DataPtr, int Count, uint PreviousCrc32) =>
			BitConverter.IsLittleEndian ?
				crc32LittleEndian(DataPtr, Count, PreviousCrc32)
				: crc32BigEndian(DataPtr, Count, PreviousCrc32);

		private static unsafe uint crc32LittleEndian(byte* DataPtr, int Count, uint PreviousCrc32)
		{
			uint crc = ~PreviousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
			int unroll = 4;
			int bytesAtOnce = 16 * unroll;

			fixed (uint* lookup0 = crc32Lookup)
			{
				uint* uintPtr = (uint*)DataPtr;
				uint* uintEndPtr = uintPtr + ((Count / bytesAtOnce) * (bytesAtOnce / 4));

				uint* lookup1 = lookup0 + 0x100;
				uint* lookup2 = lookup0 + 0x200;
				uint* lookup3 = lookup0 + 0x300;
				uint* lookup4 = lookup0 + 0x400;
				uint* lookup5 = lookup0 + 0x500;
				uint* lookup6 = lookup0 + 0x600;
				uint* lookup7 = lookup0 + 0x700;
				uint* lookup8 = lookup0 + 0x800;
				uint* lookup9 = lookup0 + 0x900;
				uint* lookup10 = lookup0 + 0xA00;
				uint* lookup11 = lookup0 + 0xB00;
				uint* lookup12 = lookup0 + 0xC00;
				uint* lookup13 = lookup0 + 0xD00;
				uint* lookup14 = lookup0 + 0xE00;
				uint* lookup15 = lookup0 + 0xF00;
				...
				while (uintPtr < uintEndPtr)
				{
					for (int unrolling = 0; unrolling < unroll; ++unrolling)
					{
						...
						uint three = *uintPtr++;
						uint four = *uintPtr++;
						crc = lookup0[(four >> 24) & 0xFF] ^
							lookup1[(four >> 16) & 0xFF] ^
							lookup2[(four >> 8) & 0xFF] ^
							lookup3[four & 0xFF] ^
							lookup4[(three >> 24) & 0xFF] ^
							lookup5[(three >> 16) & 0xFF] ^
							lookup6[(three >> 8) & 0xFF] ^
							lookup7[three & 0xFF] ^
							lookup8[(two >> 24) & 0xFF] ^
							lookup9[(two >> 16) & 0xFF] ^
							lookup10[(two >> 8) & 0xFF] ^
							lookup11[two & 0xFF] ^
							lookup12[(one >> 24) & 0xFF] ^
							lookup13[(one >> 16) & 0xFF] ^
							lookup14[(one >> 8) & 0xFF] ^
							lookup15[one & 0xFF];
					}
				}
				...
				{
					crc = (crc >> 8) ^ *(lookup0 + ((crc & 0xFF) ^ DataPtr[i++]));
				}
			}
		...
		private static unsafe uint crc32BigEndian(byte* DataPtr, int Count, uint PreviousCrc32)
		{
			uint crc = ~PreviousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
			const int UNROLL = 4;
			const int BYTES_AT_ONCE = 16 * UNROLL;

			fixed (uint* lookup0 = crc32Lookup)
			{
				uint* uintPtr = (uint*)DataPtr;
				uint* uintEndPtr = uintPtr + ((Count / BYTES_AT_ONCE) * (BYTES_AT_ONCE / 4));
				...
				while (uintPtr < uintEndPtr)
				{
					...
					{
						//Big endian
						uint one = *uintPtr++ ^ _Swap(crc);
						uint two = *uintPtr++;
						uint three = *uintPtr++;
						uint four = *uintPtr++;
						crc = lookup0[four & 0xFF] ^
							lookup1[(four >> 8) & 0xFF] ^
							lookup2[(four >> 16) & 0xFF] ^
							lookup3[(four >> 24) & 0xFF] ^
							lookup4[three & 0xFF] ^
							lookup5[(three >> 8) & 0xFF] ^
							lookup6[(three >> 16) & 0xFF] ^
							lookup7[(three >> 24) & 0xFF] ^
							lookup8[two & 0xFF] ^
							lookup9[(two >> 8) & 0xFF] ^
							lookup10[(two >> 16) & 0xFF] ^
							lookup11[(two >> 24) & 0xFF] ^
							lookup12[one & 0xFF] ^
							lookup13[(one >> 8) & 0xFF] ^
							lookup14[(one >> 16) & 0xFF] ^
							lookup15[(one >> 24) & 0xFF];
					}
				}

				// remaining 1 to 63 bytes (standard algorithm)
				for (int i = (Count / BYTES_AT_ONCE) * BYTES_AT_ONCE; i < Count;)
				{
					crc = (crc >> 8) ^ lookup0[(crc & 0xFF) ^ DataPtr[i++]];
				}
			}
		...
		/// swap endianess
		private static uint _Swap(uint X) =>
			(X >> 24) |
			((X >> 8) & 0x0000FF00) |
			((X << 8) & 0x00FF0000) |
			(X << 24);
